...
__global__ void kernel_compute(float *a, float *b, int N)
{
	...
	auto f[2];
	f[0] = [](const int& a, const int& b) -> bool
    		{ 
        		return a > b; 
    		};
	f[1] = [](const int& a, const int& b) -> bool
    		{ 
        		return a < b; 
    		};
	printf("%d\n", f[0](1, 3), f[1](1, 3));
...
int compute_UnifiedMem(void)
{
	...
	cudaMallocManaged(reinterpret_cast<void **>(&host_a),
			  sizeof(float) * NUM_ELEM);
	cudaMallocManaged(reinterpret_cast<void **>(&host_b),
			  sizeof(float) * NUM_ELEM);
...
int compute_NoUnifiedMem(void)
{
	...
	cudaMalloc(reinterpret_cast<void **>(&device_a), NUM_ELEM * sizeof(float));
	cudaMalloc(reinterpret_cast<void **>(&device_b), NUM_ELEM * sizeof(float));
	...
	kernel_compute << < NUM_ELEM / 256, 256 >> >
					    (device_a, device_b, NUM_ELEM);
